...
10.【当前时间校准】今天是${new Date().toLocaleDateString('zh-CN',{year:'numeric',month:'long',day:'numeric',weekday:'long'})}，当前为${new Date().getMonth()+1}月，属于${['冬季（水旺）','冬季（水旺）','春季（木旺）','春季（木旺）','春季（木旺）','夏季（火旺）','夏季（火旺）','夏季（火旺）','秋季（金旺）','秋季（金旺）','秋季（金旺）','冬季（水旺）'][new Date().getMonth()]}。今天是${new Date().getFullYear()}年${new Date().getMonth()+1}月，不是其他年月。判断五行旺衰和调候时必须以此当前季节为准，不要说错年月和季节！
...
要求：
- 全程大白话，不说任何专业术语
- 今天是${new Date().toLocaleDateString('zh-CN',{year:'numeric',month:'long',day:'numeric',weekday:'long'})}，${new Date().getMonth()+1}月，属于${['冬季','冬季','春季','春季','春季','夏季','夏季','夏季','秋季','秋季','秋季','冬季'][new Date().getMonth()]}，不准说错当前日期和季节
- 要具体、有画面感、能让人用得上
- 温暖不油腻，直接不冷硬
`;

// ══════════════════════════════════════════════════════════════════
// 本地排盘（Python 子进程，<1秒）——不再让 LLM 手算
// ══════════════════════════════════════════════════════════════════
function computeBaziLocally(body) {
  const computeScript = join(__dirname, 'api', 'bazi_compute.py');
  if (!existsSync(computeScript)) return null;
  try {
    const { birthDate, birthTime, birthPlace, gender } = body;
    if (!birthDate) return null;
    const [y, m, d] = birthDate.split('-').map(Number);
    const [h, min = '0'] = (birthTime || '12:00').split(':').map(Number);
    const cityLng = {
      '北京':116.4,'上海':121.5,'广州':113.3,'深圳':114.1,'杭州':120.2,
      '南京':118.8,'成都':104.1,'重庆':106.5,'武汉':114.3,'西安':108.9,
      '郑州':113.7,'济南':117.0,'青岛':120.4,'天津':117.2,'苏州':120.6,
      '长沙':113.0,'福州':119.3,'厦门':118.1,'昆明':102.7,'贵阳':106.7,
      '沈阳':123.4,'大连':121.6,'哈尔滨':126.6,'长春':125.3,'太原':112.5,
      '石家庄':114.5,'合肥':117.3,'南昌':115.9,'南宁':108.4,'海口':110.3,
      '兰州':103.8,'西宁':101.8,'银川':106.3,'乌鲁木齐':87.6,'拉萨':91.1,
      '呼和浩特':111.7,'台北':121.5,'香港':114.2,'澳门':113.5,
    };
    let lng = 120.0;
    if (birthPlace) {
      for (const [k, v] of Object.entries(cityLng)) {
        if (birthPlace.includes(k)) { lng = v; break; }
      }
    }
    const input = JSON.stringify({ name: body.name || '匿名', gender: gender || '未知', year: y, month: m, day: d, hour: h, minute: min, longitude: lng });
    const raw = execSync(`cd "${__dirname}" && python3 api/bazi_compute.py`, { input, timeout: 5000, maxBuffer: 1024 * 1024 });
    const result = JSON.parse(raw.toString());
    if (result.error) { console.error('Bazi compute error:', result.error); return null; }
    return result;
  } catch (e) {
    console.error('Local bazi compute failed:', e.message);
    return null;
  }
}

function formatBaziForLLM(local) {
  const sz = local.sizhu;
  const dy = local.dayun;
  const mg = local.minggong;
  const kw = local.kongwang;
  let text = '【已通过本地引擎精确计算的命盘数据，无需重新排盘】\n';
  text += `八字四柱：${sz.year.ganzhi} ${sz.month.ganzhi} ${sz.day.ganzhi} ${sz.hour.ganzhi}\n`;
  text += `日主：${local.rizhu_summary.day_tg}（${local.rizhu_summary.day_tg_wuxing}性，${local.rizhu_summary.day_tg_yinyang}干）\n`;
  text += `年柱 ${sz.year.ganzhi}：天干${sz.year.tg}(${sz.year.tg_wuxing}) 地支${sz.year.dz}(${sz.year.dz_wuxing}) 藏干${sz.year.canggan.join(' ')} 十神${sz.year.tg_shishen}\n`;
  text += `月柱 ${sz.month.ganzhi}：天干${sz.month.tg}(${sz.month.tg_wuxing}) 地支${sz.month.dz}(${sz.month.dz_wuxing}) 藏干${sz.month.canggan.join(' ')} 十神${sz.month.tg_shishen}\n`;
  text += `日柱 ${sz.day.ganzhi}：天干${sz.day.tg}(${sz.day.tg_wuxing}) 地支${sz.day.dz}(${sz.day.dz_wuxing}) 十神=日主\n`;
  text += `时柱 ${sz.hour.ganzhi}：天干${sz.hour.tg}(${sz.hour.tg_wuxing}) 地支${sz.hour.dz}(${sz.hour.dz_wuxing}) 藏干${sz.hour.canggan.join(' ')} 十神${sz.hour.tg_shishen}\n`;
  text += `命宫：${mg.ganzhi}（${mg.tg}${mg.dz}）\n`;
  text += `空亡：${kw.dizhi.join('、')}（${kw.xun_head}旬）\n`;
  text += `起运：${dy.qiyun_age}岁 ${dy.shunni}\n`;
  text += `大运：${dy.list.map(d => `${d.start_age}-${d.end_age}岁 ${d.ganzhi}`).join(' / ')}\n`;
  if (dy.current) text += `当前大运：${dy.current.ganzhi}（${dy.current.start_age}-${dy.current.end_age}岁）\n`;
  return text;
}
...
async function proxyToDeepSeek(req, res) {
  ...
  switch (effectiveType) {
    case 'full': {
      // 🚀 先用本地引擎秒算排盘，再喂给 LLM 做白话解读
      const localBazi = computeBaziLocally(body);

      if (localBazi) {
        // 本地算出来了 → LLM 只需翻译成人话
        systemPrompt = SYSTEM_SHORT + `\n\n用户命盘已由本地引擎精确计算（不是AI推测），你只需用大白话向用户解读。`;
        const baziData = formatBaziForLLM(localBazi);
        const todayStr = new Date().toLocaleDateString('zh-CN',{year:'numeric',month:'long',day:'numeric',weekday:'long'});
        userMsg = `以下是本地引擎已精确算出的命盘数据：
${baziData}

出生日期：${birthDate}（${birthDate ? birthDate.split('-')[1] : '?'}月，属${['冬季','冬季','春季','春季','春季','夏季','夏季','夏季','秋季','秋季','秋季','冬季'][parseInt(birthDate ? birthDate.split('-')[1] : '1') - 1]}）
出生时间：${birthTime || '12:00'}${timeUnknown ? '（时辰不确定，按中午12点算，可能有偏差）' : ''}
出生地点：${birthPlace || '未提供'}
性别：${gender || '未提供'}
${name ? '称呼：' + name : ''}
今天是${todayStr}。

请按以下顺序用大白话解读这个命盘（像朋友聊天，不说术语）：
1. 一两句整体印象
2. 性格特点
3. 事业和财运
4. 感情和家庭
...
必须包含：
- 八字四柱（直接用上面的四柱）
- 结尾用【】单独一段附上内部校验：四家投票结论（徐/梁/袁/韦各一两个关键词）+ 紫微命宫和财帛宫主星`;
        maxTokens = 4096;
      } else {
        // 本地计算失败，回退到 LLM 手动排盘
        console.log('⚠️ 本地排盘失败，回退到 LLM 推演');
...
今天是${todayStr}，${new Date().getMonth()+1}月，属于${['冬季','冬季','春季','春季','春季','夏季','夏季','夏季','秋季','秋季','秋季','冬季'][new Date().getMonth()]}。
